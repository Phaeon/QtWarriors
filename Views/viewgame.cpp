...
void ViewGame::chooseMovementAttack()
{
    if(_x == 10 || !_isSelected)
        _x = _player->pawnsPossibles(_controller->getGameManagement()->getGame(), _player->getPion(_pawnChosen))[0]._x;

    if(_y == 10 || !_isSelected)
        _y = _player->pawnsPossibles(_controller->getGameManagement()->getGame(), _player->getPion(_pawnChosen))[0]._y;
